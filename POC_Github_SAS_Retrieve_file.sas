/************************************************************************************
*** @Author: Yacine ASLIMI
***	@Date: 13072022
***	@Subject: SAS Macro: Retrieve github folder and files on the SAS environnement
*** @Note: This program will not run properly on z/OS. Only Windows, Linux and UNIX are supported. 
		   Values for PATH are CASE SENSITIVE.
*************************************************************************************/

/************************************************************************************
****  WORKSPACE DEFINITION PATTERN: IP + user_workspaces+UserID+Dossier
*************************************************************************************/

%let path=//172.16.128.2/user_workspaces/&sysuserid/EPG194;
...
/************************************************************************************
*** @Macro: MACRO DEFINTION
*** @Steps:
		1. Variables definition
		2. Check if the file exist (ie the url is ok)
		3. Retrive the file
		4. Read the files from the zip
		5. Create the data folder structure and get a list of files in macro variables
		6. Delete .zip file in the folder
*************************************************************************************/
...
%let url=%str(https://github.com/yacaslimi/SAS/archive/refs/heads/master.zip);
...
/************************************************************************************
**** Check if the defined path exist (sas env path)
*************************************************************************************/

/* Is the path specified valid? */
%let fileref=unzip;  /*Le chemin*/
%let rc=%sysfunc(filename(fileref,%superq(unzip))); /* Le nom chemin dans rc*/
%let path_found=%sysfunc(fileref(unzip)); /* file*/


%put %sysfunc(filename(file)); /*Exploration*/
%put %sysfunc(filename(rc));
%put %sysfunc(filename(path_found));
...
%if &big_zip_found ne 0 %then %do; /*Si l'élément récupérer est non vide, alors*/
...
%put %sysfunc(filename(big_zip_found));
...
**** 4. Read the files from the zip
*************************************************************************************/
/*Recupération des données depuis le zip: Creation de dossier vides et copie des fichiers*/
...
/************************************************************************************
*** DELETE .ZIP FILE IN THE FOLDER
*************************************************************************************/

%sysfunc(fdelete(&BigZip));
